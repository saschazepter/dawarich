...
RSpec.describe 'Demo Data API', type: :request do
  ...
  path '/api/v1/demo_data' do
    get 'Checks whether demo data exists' do
      tags 'Demo Data'
      description 'Returns whether the authenticated user has demo data loaded. Responds 404 on servers without demo-data support, which clients treat as the capability signal.'
    ...
    post 'Loads demo data' do
      tags 'Demo Data'
      description 'Seeds the demo dataset (Berlin + Prague) for the authenticated user. Returns 200 with status "exists" when demo data is already loaded.'
      produces 'application/json'
      parameter name: :api_key, in: :query, type: :string, required: true, description: 'API Key'
    ...
    delete 'Removes demo data' do
      tags 'Demo Data'
      description 'Removes all demo data (points, visits, places, tags, tracks and the demo trip) for the authenticated user. Real user data is not touched. Returns status "no_demo_data" when there is nothing to remove.'
      produces 'application/json'
      parameter name: :api_key, in: :query, type: :string, required: true, description: 'API Key'
