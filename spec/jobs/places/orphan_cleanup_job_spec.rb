  ...
  describe '#perform' do
    ...
    context 'when called with nil (ownerless pass)' do
      it 'deletes ownerless orphan photon places that no per-user job can reach' do
        ownerless = create(:place, user: nil, source: :photon, name: Place::DEFAULT_NAME)
        owned_orphan = create(:place, user: user, source: :photon, name: Place::DEFAULT_NAME)
      ...
      it 'keeps ownerless places that are manual, noted, tagged, visit-backed, or renamed' do
        ...
        renamed = create(:place, user: nil, source: :photon, name: 'Home')
