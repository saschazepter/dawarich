...
RSpec.describe SharedLinks::OgImageJob do
  ...
  describe '#perform' do
    ...
    it 'skips rendering for phrase-protected links so name/dates never reach the image' do
      protected_link = create(:shared_link, :with_phrase)
      expect(SharedLinks::OgImageRenderer).not_to receive(:new)

      described_class.perform_now(protected_link.id)

      expect(protected_link.reload.og_image_state).to eq('failed')
    end
