...
RSpec.describe 'Settings::VisitDetection', type: :request do
  let(:user) { create(:user) }
  before { sign_in user }

  describe 'GET /settings/visit_detection' do
    it 'renders for any logged-in user' do
      get settings_visit_detection_path
  ...
  describe 'PATCH /settings/visit_detection' do
    it 'updates the three permitted keys' do
      patch settings_visit_detection_path, params: {
    ...
    it 'redirects on success' do
      patch settings_visit_detection_path, params: { settings: { visit_radius_meters: '60' } }
