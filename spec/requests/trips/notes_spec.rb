  ...
  describe 'POST /trips/:trip_id/notes' do
    ...
    context 'with find_or_initialize (idempotent create)' do
      ...
      it 'updates existing note instead of creating a duplicate' do
        ...
        expect(existing_note.body).to eq('Great day exploring!')
      end
    end
    ...
    context 'with missing date param' do
      it 'does not crash and does not create a note' do
        expect do
          post trip_notes_url(trip), params: { note: { body: 'Lost note' } }
        end.not_to change(Note, :count)
      end
    end

    context 'with malformed date param' do
      it 'does not crash and does not create a note' do
        expect do
          post trip_notes_url(trip), params: { note: { date: 'banana', body: 'Bad date' } }
        end.not_to change(Note, :count)
      end
    end
    ...
    context 'with empty body' do
      it 'does not save the note' do
        expect do
          post trip_notes_url(trip), params: { note: { date: trip.started_at.to_date, body: '' } }
        end.not_to change(Note, :count)
      end
    end
  ...
  describe 'PATCH /trips/:trip_id/notes/:id' do
    ...
    context 'with valid parameters' do
      it 'updates the note' do
        ...
        note.reload
        expect(note.body).to eq('Updated note content')
      end
