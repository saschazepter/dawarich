...
- While an import is still being imported or its visits, places and transportation modes are still being extracted, its points stay with it: another import from the same Takeout, the scheduled track run and the Cloud history backfill no longer turn them into plain tracks. Rebuilding tracks never deletes a track the extraction wrote or one that carries the source app's modes or your own corrections.
- Extracting data from an older import replaces Dawarich's guessed transportation modes with the source app's modes only where the source covers a track, never changes a mode you corrected by hand, and undoing the extraction brings the guessed modes back. An extraction stuck without a start time now offers Start over.
